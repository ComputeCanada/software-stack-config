...
# additions for CC (plus removing dependency on bash-completion package)

# Check for interactive bash
if [ "x${BASH_VERSION-}" == x -o "x${PS1-}" == x ]; then
	return
fi
# Not without CCCONFIG
if [ -n "$RSNT_NO_CCCONFIG" ]; then
	return
fi

if ! declare -F _completion_loader >/dev/null 2>&1; then
	# bash_completion not loaded: provide basic versions
...
if [ "$SOURCED" -eq 0 ]; then
	...
	return
elif true; then	: # CC: not dependent on bash_completion!
